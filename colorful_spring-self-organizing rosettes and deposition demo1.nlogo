...
[set breed Wnter
  ...
  set shape "hex" ]
  ;
if ( xcor >= max [xcor ] of turtles - Fgf-size  ) and ( xcor <= max [xcor ] of turtles - Wnt-size ) [set breed fgfer set shape "hex" set color yellow ]
if  xcor < max [xcor ] of turtles - fgf-size [set breed depositer set shape "hex" set color red ]
...
ask Wnter
  [ set heading 90 fd speed
    ...
    if show-density?  [ set color scale-color red  ( count turtles in-radius 3 ) 0 density-scale ]
...
[ if count wnter > 0
 [ ask one-of wnter
 [ if ( random 100  / 1000 ) < proliferation-rate [ hatch-wnter 1 [set color blue set ycor 0] ]]
...
if shrink-wnt-zone = true [ shrink-Wnt-domain ]
 ...
 if fgfer-proliferate? [ if count fgfer > 0
  [ ask one-of fgfer
  [ if ( random 100  / 1000 ) < proliferation-rate [ hatch-fgfer 1 [set color blue set ycor 0] ]]
 ]
 ]
  tick
...
494
make-movie
repeat 400\n\n[\n\nexport-view (word \"self-organizing-condensations-demo-v4-break-threshold_1- \" ticks \".png\")\ngo\n]\n
NIL
1
T
OBSERVER
